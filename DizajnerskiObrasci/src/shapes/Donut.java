...
public class Donut extends Circle {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6513204439635959461L;
	private int innerRadius;
	//private Color smallInsideColor;
	...
	/*public Donut(Point center, int radius, int innerRadius, Color smallInsideColor) throws Exception {
		this(center, radius, innerRadius);
		setSmallInsideColor(smallInsideColor);
	}*/
	
	public Donut(Point center, int radius, int innerRadius, Color insideColor, Color outsideColor) throws Exception {
		this(center, radius, innerRadius);
		setInsideCol(insideColor);
		setOutsideColor(outsideColor);
	}
	
	public Donut(Point center, int radius, int innerRadius, Color insideColor, Color outsideColor, boolean selected) throws Exception {
		this(center, radius, innerRadius, insideColor, outsideColor);
		setSelected(selected);
	 ...
	 * 
	 * PnlDrawing pozadina = new PnlDrawing(); g.setColor(pozadina.getBackground());
	 * g.fillOval(this.getCenter().getX() - this.innerRadius,
	 * this.getCenter().getY() - this.getInnerRadius(), this.getInnerRadius()*2,
	 * this.innerRadius*2);
	 * 
	 * if(smallInsideColor!=null) g.setColor(smallInsideColor); else
	 * g.setColor(Color.BLACK);
	 * 
	 * g.drawOval(this.getCenter().getX() - this.innerRadius,
	 * this.getCenter().getY() - this.getInnerRadius(), this.getInnerRadius()*2,
	 * this.innerRadius*2); g.setColor(Color.BLACK);
	 * 
	 * if (isSelected()) { g.setColor(Color.BLUE); g.drawRect(getCenter().getX() -
	 * 3, getCenter().getY() - 3, 6, 6); g.drawRect(getCenter().getX() +
	 * getInnerRadius() - 3, getCenter().getY() - 3, 6, 6);
	 * g.drawRect(getCenter().getX() - getInnerRadius() - 3, getCenter().getY() - 3,
	 * 6, 6); g.drawRect(getCenter().getX() - 3, getCenter().getY() +
	 * getInnerRadius() - 3, 6, 6); g.drawRect(getCenter().getX() - 3,
	 * getCenter().getY() - getInnerRadius() - 3, 6, 6); g.setColor(Color.BLACK); }
	 * }
	 */

	//unutrasnjost donut-a transparentna -->
	public void draw(Graphics g) {
		...
		graph.setColor(getInsideCol());
		graph.fill(bigArea);
		graph.setColor(getOutsideColor());
		graph.draw(bigArea);
	...
	public String toString() {
		return "Donut: (" + getCenter().getX() + ", " + getCenter().getY() + ") " + "outer radius = " + getRadius()
				+ "; inner radius = " + innerRadius + "; inside color {"
				+ Integer.toString(super.getInsideCol().getRGB()) + "} outside color {"
				+ Integer.toString(super.getOutsideColor().getRGB()) + "}";
	}
	...
	public Donut clone(Donut d) {
		...
		d.setInsideCol(this.getInsideCol());
